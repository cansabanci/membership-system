// ipcRenderer, readPhotoAsDataUrl, currentUserRole diğer script'lerde (ui.js/members.js) zaten
...
async function saveProfile() {
    ...
    ipcRenderer.send('update-my-profile', payload);
}
...
ipcRenderer.on('my-profile-loaded', (event, member) => {
    if (!member) return;
    fillProfileCard(member);
});

ipcRenderer.on('my-profile-updated', (event, member) => {
    if (!member) return;
    fillProfileCard(member);
...
if (typeof currentUserRole !== 'undefined' && currentUserRole === 'viewer') {
    ipcRenderer.send('get-my-profile');
}
